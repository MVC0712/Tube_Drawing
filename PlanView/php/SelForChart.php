...
$end_s = $_POST['end_s'];
  ...
  ";
    foreach ($period as $dt) {
      $di = $dt->format("Y-m-d");
      $din = $dt->format("Ymd");
      $sql1 = $sql1 ." MAX(CASE WHEN t_plan.product_date = '" . $di . "'  THEN 12*6*132 ELSE '' END)/1000 AS '_" . $din ."',";
      }
  ...
  ";
    foreach ($period as $dt) {
      $di = $dt->format("Y-m-d");
      $din = $dt->format("Ymd");
      $sql1 = $sql1 ." MAX(CASE WHEN t_casting.product_date = '" . $di . "'  THEN tc.tt1200 + tc.tt600 ELSE '' END)/1000 AS '_" . $din ."',";
      }
      $sql1 = substr(trim($sql1), 0, -1);
      $sql1 = $sql1." FROM
      t_casting
      LEFT JOIN (
        ...
        SUM(A2Q12 + A3Q12 + B1Q12 + B2Q12 + B3Q12 + B4Q12 + C1Q12 + C2Q12 + C3Q12 + C4Q12 + D2Q12 + D3Q12)*132 AS tt1200,
        SUM(A2Q6 + A3Q6 + B1Q6 + B2Q6 + B3Q6 + B4Q6 + C1Q6 + C2Q6 + C3Q6 + C4Q6 + D2Q6 + D3Q6)*66 AS tt600
        FROM t_cutting 
        GROUP BY  tcid            
